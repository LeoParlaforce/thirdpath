// src/app/api/tracks/availability/route.ts
...
type Availability = { count: number; spotsLeft: number; full: boolean }
...
const OCCUPIED = new Set<Stripe.Subscription.Status>([
  "active",
  "trialing",
  "past_due",
  "unpaid",
...
async function countByTrack(track: TrackId): Promise<number> {
  const query = `metadata['track']:'${track}' AND status:'active'`
  const first = await stripe.subscriptions.search({ query, limit: 100 })
  let total = first.data.length
  let next = first.next_page
  while (next) {
    const page = await stripe.subscriptions.search({ query, page: next, limit: 100 })
...
export async function GET() {
  const base: Record<TrackId, Availability> = {
  ...
  async function safeCount(track: TrackId) {
    try {
      ...
      // fallback if search is unavailable
      const subIds = new Set<string>()
      let starting_after: string | undefined
      while (true) {
        const page = await stripe.subscriptions.list({ limit: 100, starting_after })
        for (const s of page.data) {
          if (!OCCUPIED.has(s.status) || s.cancel_at_period_end) continue
